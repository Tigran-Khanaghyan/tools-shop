    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col group hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
      ...
      <div className="relative bg-gray-50 overflow-hidden aspect-[4/3]">
        <img
          ...
          onError={(e) => {
            (e.target as HTMLImageElement).src = `https://placehold.co/400x300/f1f5f9/94a3b8?text=${encodeURIComponent(tool.name)}`;
          }}
        />
        <span className="absolute top-2 left-2 inline-flex items-center gap-1 bg-white/90 backdrop-blur-sm text-xs font-medium text-gray-600 px-2 py-0.5 rounded-full border border-gray-200">
          <Tag size={10} />
          {tool.category}
        </span>
      </div>
      ...
      <div className="flex flex-col flex-1 p-4 gap-3">
        <div className="flex-1">
          <h3 className="font-semibold text-gray-900 text-base leading-snug mb-1">{tool.name}</h3>
